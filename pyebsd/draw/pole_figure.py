...
def draw_projection(ax, d, **kwargs):
    ...
    c0, c1 = d[0] / (1.0 + d[2]), d[1] / (1.0 + d[2])
    ax.plot(c0, c1, **kw)

    return ax


def draw_circle(ax, n, ang=90, r=1.0, p=1.0, **kwargs):
    """
    ...
    if (n[0] == 0.0) & (n[1] == 0.0):
        ...
        b = np.asarray([-n[0] * n[2], -n[1] * n[2], n[0] ** 2 + n[1] ** 2]).reshape(
            3, 1
        )
...
def draw_trace(ax, n=[1, 0, 1], offset=0.0, r=1.0, p=1.0, **kwargs):
    n = np.asarray(n) / np.linalg.norm(n)  # normalize n
    ...
    C = C.reshape(3, 1)
    r_prime = (r**2.0 - offset**2.0) ** 0.5  # Radius of the circle
...
def draw_wulff_net(ax, step=9.0, theta=0.0, n=None, **kwargs):
    """
    Parameters
    ----------
    ...
    kw = dict(c="k", lw=".5")
    kw.update(kwargs)
    for t in np.arange(-90.0, 90.0, step)[1:] * np.pi / 180.0:
        # draw latitude traces
        draw_trace(ax, n=[-stheta, ctheta, 0], offset=np.sin(t), **kw)
        draw_trace(
            ax, n=[ctheta * np.cos(t), stheta * np.cos(t), np.sin(t)], **kw
        )  # draw longitude traces
...
def initialize_frame_PF(ax, **kwargs):
    ax.set_aspect("equal")
    ax.axis("off")
    draw_circle_frame(ax, lw=0.5)
    draw_std_traces(ax, lw=0.5)
