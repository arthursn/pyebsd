...
__all__ = [
    ...
    "stereographic_projection_to_direction",
    "average_orientation",
    "misorientation",
    "misorientation_neighbors",
    "kernel_average_misorientation",
    "minimize_disorientation",
    "euler_angles_to_rotation_matrix",
    "rotation_matrix_to_euler_angles",
    "axis_angle_to_rotation_matrix",
    "list_cubic_symmetry_operators_KS",
    "list_cubic_symmetry_operators",
    "list_cubic_family_directions",
...
def stereographic_projection_to_direction(xy):
    ...
    uvw[:, 2] = (1.0 - x**2.0 - y**2.0) / (1.0 + x**2.0 + y**2.0)
    uvw[:, 0] = x * (1.0 + uvw[:, 2])
    uvw[:, 1] = y * (1.0 + uvw[:, 2])
...
def average_orientation(M, sel=None, **kwargs):
    ...
    **kwargs :
    ...
    if kwargs.get("vectorized", True):
        ...
        Mprime = np.tensordot(C, M_sel, axes=[[-1], [-2]]).transpose([2, 0, 1, 3])
        ...
        for i in range(N):
            ...
            neg = tr < 0.0  # select negative traces
            tr[neg] = -tr[neg]
            Mprime[neg] = -Mprime[neg]
...
def misorientation(A, B, out="deg"):
    ...
    if (Adim == 2) and (Bdim == 2):
        ...
        if x > 3.0:
            x = 3.0
        if out != "tr":
            x = np.arccos((x - 1.0) / 2.0)  # mis x in radians
            if out == "deg":
...
def kernel_average_misorientation(
    M, neighbors, sel=None, maxmis=None, out="deg", **kwargs
):
    """
...
def minimize_disorientation(V, V0, **kwargs):
    ...
    maxdev = kwargs.pop("maxdev", 0.25)  # maximum deviation in degrees
    ...
    if plot:
        ...
        axmin = plot_PF(M=V, ms=0.5, c="k", verbose=False)
    for i in range(it):
        step = maxdev / n
        ...
        D = np.tensordot(V, B.transpose(0, 2, 1), axes=[[-1], [-2]]).transpose(
            [0, 2, 1, 3]
        )
        ...
        imax = np.argmax(tr)
        if verbose:
            dth = np.degrees(np.arccos((np.trace(A[imax]) - 1.0) / 2.0))
            sys.stdout.write(
                "{:2d} : {:g}, {:g}, {:g}; mis = {:g} deg\n".format(
                    i + 1,
                    np.degrees(theta[imax]),
                    np.degrees(phi[imax]),
                    np.degrees(psi[imax]),
                    dth,
                )
    ...
    **kwargs :
    ...
    if not kwargs.pop("avg", False):
        ...
        if np.ndim(neg1) > 0:
            ...
            phi1[neg1] = phi1[neg1] + 2.0 * np.pi
            phi2[neg2] = phi2[neg2] + 2.0 * np.pi
        else:
            if neg1:
                phi1 += 2.0 * np.pi
            if neg2:
...
def axis_angle_to_rotation_matrix(axis, theta):
    ...
    R[:, 0, 0] = ctheta1 * axis[:, 0] ** 2.0 + ctheta
    ...
    R[:, 1, 1] = ctheta1 * axis[:, 1] ** 2.0 + ctheta
    ...
    R[:, 2, 2] = ctheta1 * axis[:, 2] ** 2.0 + ctheta
...
def list_cubic_symmetry_operators_KS():
    ...
    return (
        axis_angle_to_rotation_matrix(axis_angle[:, 1:], axis_angle[:, 0])
        .round(0)
        .astype(int)
    )
...
def reduce_cubic_transformations(V, maxdev=1e-3):
    ...
    maxdev = 2.0 * np.cos(np.radians(maxdev)) + 1.0
    C = list_cubic_symmetry_operators()
