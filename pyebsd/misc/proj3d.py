...
def plot_sproj3d(n=[1, 0, 1], offset=0.0, **kwargs):
    n = np.asarray(n) / np.linalg.norm(n)  # normalize n

    r = kwargs.pop("r", 1.0)  # radius of the sphere
    ...
    Q[0], Q[1], Q[2] = p * P[0] / (p + P[2]), p * P[1] / (p + P[2]), 0.0
    ...
    xp, yp = p * n[0] / (p + n[2]), p * n[1] / (
        p + n[2]
    )  # stereographic projection of the pole
    ax.plot([xp], [yp], [0], "go")
...
if __name__ == "__main__":
    ...
    ax = plot_sproj3d(n=[1.0, 1.0, 1.0], offset=0.95, p=0.0)
