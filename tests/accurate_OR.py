...
mapks = scan.plot_property(mis_KS, sel=filt & (
    scan.ph == 1), vmin=0, vmax=5., gray=scan.IQ, tiling='hex')
...
ax = pyebsd.plot_PF(R=scan.R[filt & (scan.ph == 2)], contour=True, cmap=plt.get_cmap(
    'Blues_r'), bins=512, nlevels=20, fn=None, proj=proj0, parent_or=R_fcc)
pyebsd.plot_PF(M=V, contour=True, cmap=plt.get_cmap('Reds_r'),
               bins=512, nlevels=20, fn=None, proj=proj, ax=ax)
# ax = pyebsd.plot_PF(R=scan.R[filt & (scan.ph == 2)], c="b", proj=proj0, parent_or=R_fcc)
# pyebsd.plot_PF(M=V, c="r", proj=proj, ax=ax)
pyebsd.plot_PF(M=np.dot(V0, C).transpose([1, 0, 2]), mfc=[
               1, 1, 1, 0], mec='k', mew=1, marker='o', ms=3, proj=proj, verbose=False, ax=ax)
...
ax2 = pyebsd.plot_PF(M=np.dot(V0, C).transpose([1, 0, 2]), mfc=[
                     1, 1, 1, 0], mec='k', mew=1, marker='o', ms=5, proj=proj, label='exp.')
pyebsd.plot_PF(M=T_KS, mfc=[1, 1, 1, 0], mec='r', mew=1, marker='s', ms=5,
               proj=proj, parent_or=parent_or, verbose=False, ax=ax2, label='KS')
pyebsd.plot_PF(M=T_NW, mfc=[1, 1, 1, 0], mec='g', mew=1, marker='D', ms=5,
               proj=proj, parent_or=parent_or, verbose=False, ax=ax2, label='NW')
pyebsd.plot_PF(M=T_GT, mfc=[1, 1, 1, 0], mec='b', mew=1, marker='v', ms=5,
               proj=proj, parent_or=parent_or, verbose=False, ax=ax2, label='GT')
