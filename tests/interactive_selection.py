...
# fname = 'D:\\Documents\\Tohoku2016\\EBSD\\ang\\ADI300-15(2016-8-3).ang'
...
while key is not 'q':
    if np.count_nonzero(ipf.sel) > 0:
        R_fcc = pyebsd.avg_orientation(scan.R, sel=ipf.sel & (scan.ph==2) & (scan.CI>.2), vect=False, verbose=True)#, plot=True, n=10, maxdev=10., it=5)

        # plot IPF of the selected data
        ax2.cla() # clear axis
        scan.plot_IPF(sel=ipf.sel & (scan.ph==1) & (scan.CI>.3), gray=scan.IQ, tiling='hex', ax=ax2)
        ...
        scan.plot_PF(sel=ipf.sel & (scan.ph==1) & (scan.CI>.2), proj=proj, contour=True, cmap=plt.get_cmap('Reds_r'), ax=ax3, parent_or=R_fcc)
        scan.plot_PF(sel=ipf.sel & (scan.ph==2) & (scan.CI>.2), proj=proj, contour=True, cmap=plt.get_cmap('Blues_r'), ax=ax3, parent_or=R_fcc)
